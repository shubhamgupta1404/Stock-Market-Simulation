...
{
    //echo "SELECT `last_data` FROM `".$db."`.`stocks` WHERE `name`='".$row_buy['stock']."' <br />";
    $query1_buy = mysql_query("SELECT `last_data` FROM `".$db."`.`stocks` WHERE `name`='".$row_buy['stock']."'");
    $query_count++;
    ...
    //echo "UPDATE `sms`.`buy` SET `".$db."`.`success` = '1' WHERE `buy`.`sl` = ".$row_buy['sl'];
    $query_count++;
    $query_exec++;
    }
}
...
{
    //echo "SELECT `last_data` FROM `".$db."`.`stocks` WHERE `name`='".$row_sell['stock']."' <br />";
    $query1_sell = mysql_query("SELECT `last_data` FROM `".$db."`.`stocks` WHERE `name`='".$row_sell['stock']."'");
    $query_count++;
    ...
    //echo "UPDATE `sms`.`sell` SET `".$db."`.`success` = '1' WHERE `sell`.`sl` = ".$row_sell['sl'];
    $query_count++;
    $query_exec++;
    }
}
