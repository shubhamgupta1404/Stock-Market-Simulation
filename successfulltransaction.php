	...
	<tr>
		...
		<div id="content-table-inner">
			...
			<div id="table-content">
				...
				<?php
				$i = 1;
				$query = mysql_query("SELECT * FROM `".$db."`.`sell` WHERE `username`='".$_SESSION['username']."' AND `success`='1' ORDER BY `sell`.`time` DESC");
				...
				<?php
				$i = 1;
				$query = mysql_query("SELECT * FROM `".$db."`.`buy` WHERE `username`='".$_SESSION['username']."' AND `success`='1' ORDER BY `buy`.`time` DESC");
				while($row=mysql_fetch_array($query))
				{
				echo "
				<tr>
